...
CACHE_DIR = os.path.join(backend_dir, "cache_data")  # 统一缓存目录
history_file = os.path.join(backend_dir, "news_history.xlsx")  # 历史文章数据文件
...
# ================== 初始化模型 ==================
# 中文BERT模型（关键词提取）- 直接加载而非懒加载
print("正在初始化加载BERT模型...")
try:
    _bert_tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
    _bert_model = BertModel.from_pretrained('bert-base-chinese').eval().to(device)
    print("BERT模型加载成功")
except Exception as e:
...
def get_bert_model():
    ...
    if _bert_tokenizer is None or _bert_model is None:
        try:
            ...
            print(f"BERT模型重新加载失败: {e}")
    
    return _bert_tokenizer, _bert_model

# Sentence-BERT模型（相似度计算）- 直接加载而非懒加载
print("正在初始化加载Sentence-BERT模型...")
try:
    ST_MODEL_NAME = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
    _st_tokenizer = AutoTokenizer.from_pretrained(ST_MODEL_NAME)
    _st_model = AutoModel.from_pretrained(ST_MODEL_NAME).eval().to(device)
...
def get_st_model():
    """返回已加载的Sentence-BERT模型"""
    global _st_tokenizer, _st_model
    
    # 如果初始加载失败，尝试重新加载
    if _st_tokenizer is None or _st_model is None:
        try:
            print("重新尝试加载Sentence-BERT模型...")
            ST_MODEL_NAME = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
            _st_tokenizer = AutoTokenizer.from_pretrained(ST_MODEL_NAME)
            _st_model = AutoModel.from_pretrained(ST_MODEL_NAME).eval().to(device)
            print("Sentence-BERT模型重新加载成功")
        except Exception as e:
            print(f"Sentence-BERT模型重新加载失败: {e}")
    
    return _st_tokenizer, _st_model
...
def batch_text_embedding(texts):
    """批量生成文本向量"""
    # 获取Sentence-BERT模型
    st_tokenizer, st_model = get_st_model()
    if st_tokenizer is None or st_model is None:
        print("错误：Sentence-BERT模型未加载，无法生成文本向量")
        # 返回零向量
    ...
    if os.path.exists(cache_path):
        try:
            ...
            if embeddings.shape[1] != 384:
                print(f"警告：缓存的向量维度 {embeddings.shape[1]} 与预期的 384 不符，将重新计算")
            else:
                return embeddings
        except Exception as e:
            print(f"加载缓存的向量时出错: {e}，将重新计算")
...
def load_news_from_db(db_session, News, top_n=None):
    ...
    try:
        ...
        if es_client and es_client.ping():
                ...
                query = {
                    "query": {
                        "bool": {
                            ...
                            "filter": [
                                {"range": {"upload_time": {"gte": cutoff_date_str}}}
                            ]
                        }
                    },
                    "size": top_n if top_n else 1000
                }
            else:
                # 不带时间过滤的查询
                query = {
            ...
            # 执行查询
            try:
                response = es_client.search(index=NEWS_INDEX_NAME, body=query)
                hits = response.get("hits", {}).get("hits", [])
                
                if not hits:
                    print("Elasticsearch中没有找到新闻数据，将使用传统方法")
                else:
                    print(f"从Elasticsearch加载了 {len(hits)} 条新闻记录")
                    
                    # 构建DataFrame
                    data = []
                    for hit in hits:
                        source = hit["_source"]
                        news_id = source.get("news_id", hit["_id"])
                        ...
                        record = {
                            ...
                            '新闻内容': source.get("content", ""),
                            '主题': source.get("topic", "社会"),
                            '分类结果': source.get("sentiment", "中性"),
                            'embedding': vector,
                            'upload_time': source.get("upload_time", datetime.now().isoformat())
                        }
                        data.append(record)
                ...
                print(f"从Elasticsearch查询新闻数据时出错: {e}")
    except Exception as e:
        print(f"尝试使用Elasticsearch时出错: {e}")
    
    # 如果Elasticsearch不可用或查询失败，使用传统方法
    print("使用传统数据库方法加载新闻数据")
...
def recommend_articles(input_title, input_content, db_session=None, News=None, history_file="news_history.xlsx", top_n=5, use_db=False):
    """优化后的推荐函数，支持从Excel文件或数据库加载文章，并使用Elasticsearch向量数据库进行检索
    ...
    # 首先尝试使用Elasticsearch进行向量检索
    try:
        from ..utils.es_utils import es_client, NEWS_INDEX_NAME, search_by_text_embedding
        
        # 检查Elasticsearch是否可用
        es_available = es_client and es_client.ping()
        if es_available:
            ...
            # 使用Elasticsearch搜索相似文章
            es_results = search_by_text_embedding(input_emb, top_k=min(top_n * 3, 100), index_name=NEWS_INDEX_NAME)
            
            if es_results:
                print(f"Elasticsearch检索到 {len(es_results)} 条相似文章")
                ...
                if days > 0:
                    ...
                    for item in es_results:
                        ...
                        if "upload_time" in item:
                            try:
                                item_date = datetime.fromisoformat(item["upload_time"].replace("Z", "+00:00"))
                    ...
                    print(f"时间过滤前: {len(es_results)}条记录，过滤后: {len(filtered_results)}条记录")
                ...
                # 为每个结果计算加权得分
                for item in es_results:
                    # 内容相似度（已由Elasticsearch计算）
                    content_score = item["score"] - 1.0  # ES的余弦相似度计算加了1.0
                    
                    # 主题匹配得分
                    theme_score = 1 if item.get("topic") == topic_result else 0
                    
                    # 情感匹配得分
                    sentiment_score = 1 if item.get("sentiment") == sentiment_result else 0
                    
                    # 计算综合得分
                    combined_score = content_weight * content_score + theme_weight * theme_score + sentiment_weight * sentiment_score
                    item["combined_score"] = combined_score
                
                # 按综合得分排序
                sorted_results = sorted(es_results, key=lambda x: x.get("combined_score", 0), reverse=True)
                ...
                for i, item in enumerate(sorted_results[:top_n]):
                    # 构建结果对象
                    result = {
                        "id": item.get("news_id", item.get("id", i)),
                        "title": item.get("title", ""),
                        "content": item.get("content", ""),
                        "topic": item.get("topic", ""),
                        "sentiment": item.get("sentiment", ""),
                        "similarity": round(float(item.get("combined_score", item.get("score", 0))), 4)
                    }
                    
                    # 添加上传时间（如果有）
                    if "upload_time" in item:
                        result["upload_time"] = item["upload_time"]
    ...
    # 如果Elasticsearch检索失败，回退到传统方法
    print("使用传统方法从文件或数据库加载文章")
...
if __name__ == "__main__":
    ...
    recommendations = recommend_articles(sample_title, sample_content,history_file=history_file)
